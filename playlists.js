...
function apiCreatePlaylist(name) {
  const username = getUsername();
  if (!username) return Promise.reject(new Error("Not logged in."));

  const store = getPlaylistsStore();
  const list = Array.isArray(store[username]) ? store[username] : [];

  const trimmed = String(name || "").trim();
  if (!trimmed) return Promise.reject(new Error("Please enter a playlist name."));

  const dup = list.some(
    (p) => String(p.name || "").trim().toLowerCase() === trimmed.toLowerCase()
  );
  if (dup) return Promise.reject(new Error("A playlist with this name already exists."));
...
function apiDeletePlaylist(playlistId) {
  const username = getUsername();
  if (!username) return Promise.reject(new Error("Not logged in."));

  const store = getPlaylistsStore();
  const list = Array.isArray(store[username]) ? store[username] : [];
  const next = list.filter((p) => String(p.id) !== String(playlistId));
  if (next.length === list.length) return Promise.reject(new Error("Playlist not found."));
...
function apiDeleteVideo(playlistId, videoId) {
  const username = getUsername();
  if (!username) return Promise.reject(new Error("Not logged in."));
  ...
  pl.videos = videos.filter(
    (v) => !(String(v.type || "video") !== "mp3" && String(v.videoId || "") === String(videoId))
  );
  if (pl.videos.length === before) return Promise.reject(new Error("Video not found in playlist."));

  savePlaylistsStore(store);
  return Promise.resolve(true);
}

function apiSetRating(playlistId, videoId, rating) {
  const username = getUsername();
  if (!username) return Promise.reject(new Error("Not logged in."));

  const r = Number(rating);
  if (!Number.isFinite(r) || r < 0 || r > 5) return Promise.reject(new Error("Invalid rating."));
  ...
  const item = (pl.videos || []).find(
    (v) => String(v.type || "video") !== "mp3" && String(v.videoId || "") === String(videoId)
  );
  if (!item) return Promise.reject(new Error("Video not found in playlist."));
...
function apiUploadMp3() {
  return Promise.reject(new Error("MP3 upload is not available (Part A – localStorage only)."));
}

function apiDeleteMp3(playlistId, mp3Id) {
  const username = getUsername();
  if (!username) return Promise.reject(new Error("Not logged in."));
  ...
  pl.videos = (pl.videos || []).filter(
    (v) => !(String(v.type || "") === "mp3" && String(v.mp3Id || "") === String(mp3Id))
  );
  if (pl.videos.length === before) return Promise.reject(new Error("MP3 not found in playlist."));

  savePlaylistsStore(store);
  return Promise.resolve(true);
}

function apiSetMp3Rating(playlistId, mp3Id, rating) {
  const username = getUsername();
  if (!username) return Promise.reject(new Error("Not logged in."));
  ...
  if (!item) return Promise.reject(new Error("MP3 not found in playlist."));
