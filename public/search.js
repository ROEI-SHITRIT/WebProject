...
async function apiCreatePlaylist(name) {
  ...
  if (!res.ok) {
    if (data.error === "playlist_name_exists") throw new Error("Playlist name already exists.");
...
async function apiAddVideoToPlaylist(playlistId, video) {
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    if (data.error === "already_in_playlist") throw new Error("already_in_playlist");
    if (data.error === "playlist_not_found") throw new Error("playlist_not_found");
    throw new Error("Failed to add video");
  }
  return data.item;
}

/* ---------- In-memory state (server source of truth) ---------- */
let currentUser = null;
let pendingVideo = null;
let playlistsCache = [];

/* ---------- Helpers on cached playlists ---------- */
      ...
      <div class="card h-100 shadow-sm position-relative">
        ...
        <div class="card-body d-flex flex-column">
          ...
          <div class="small text-muted mb-2">
            Duration: ${escapeHtml(v.duration || "N/A")} | Views: ${escapeHtml(v.views || "N/A")}
          </div>
          ...
          <div class="mt-auto d-grid gap-2">
            ...
            <button
              class="btn btn-sm ${already ? "btn-secondary" : "btn-success"}"
              type="button"
              data-action="add"
              data-video-id="${escapeHtml(vid)}"
              data-title="${titleEsc}"
              data-thumb="${escapeHtml(v.thumbnailUrl || "")}"
              ${already ? "disabled" : ""}>
              Add to favorites
            </button>
          </div>
...
function markCardAsSaved(videoId) {
  const btn = document.querySelector(
    `button[data-action="add"][data-video-id="${CSS.escape(String(videoId))}"]`
  );
  if (!btn) return;
...
async function fetchVideosDetailsByIds(videoIds) {
  ...
  (data.items || []).forEach((item) => {
    ...
    map.set(String(id), {
      duration: isoDurationToClock(isoDur),
...
async function runSearch(query) {
  ...
  if (!YOUTUBE_API_KEY) {
    renderResults(currentUser, getMockResults(q));
    return;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  (async () => {
    ...
    try {
      ...
      console.error(err);
      playlistsCache = [];
      setAlert("Could not load playlists from server.");
    }
    ...
    confirmBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        const link = `playlists.html?playlistId=${encodeURIComponent(playlistId)}`;
        showToast(`Video saved. <a href="${link}">Open playlist</a>.`);
